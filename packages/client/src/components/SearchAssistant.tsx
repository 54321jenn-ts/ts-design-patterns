...
function SearchAssistant({ isOpen, onClose, onBuildFilters, onAddFilter, activeFilters = [], lastRemovedFilter, onFilterRemovalHandled }: SearchAssistantProps) {
  ...
  const handleSuggestFilters = () => {
    ...
    // Add each suggested filter
    suggestedFilters.forEach(filterName => {
      if (onAddFilter) {
    ...
    const assistantMessage: Message = {
      id: (Date.now() + 1).toString(),
      type: 'assistant',
      content: `I've added the most commonly used filters: File name, Created date, and Instrument. ${remainingFilters.length > 0 ? 'Would you like to add any more?' : ''}`,
      suggestions: remainingFilters,
    };
